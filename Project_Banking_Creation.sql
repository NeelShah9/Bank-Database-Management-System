...
--Used to insert the values into the Customer Table
...
--Here 'D' is used as a flag for Savings and 'C' for Checking
...
--It is executed by the trigger transaction if credit is populated as 'Y'
...
-- TRIGGER
--Invokes as soon as a row is inserted in the Transaction and makes the changes in the account table for the transaction amount
...
-- Views
 ...
 ---- Amount Versus Customer
create   view PremiumCustomers as 
 select  C.FirstName + ' ' + ISNULL(C.MiddleName,'') + ' ' + C.LastName as "Full Name"  ,
  sum(amount) Amount,c.CustomerID  
  from [dbo].[Transaction] t 
 join Account a on a.AccountNo=t.AccountNO
 join Customer c on c.CustomerID = a.CustomerID
 
 and Amount > 10000
 group by c.CustomerID , C.FirstName + ' ' + ISNULL(C.MiddleName,'') + ' ' + C.LastName 
 --order by 2 desc
 ;
...
select ZP.[State], sum(CD.TotalCreditLimit) as"Total Sanctioned Amount Across USA",
 ...
 select count(I.InsuranceID) as " Number of Insurance Accounts", MONTH(StartDate) as "Month_Name"
  ...
  ----------- Number of support tickets resolved by an Employee ----------------------
